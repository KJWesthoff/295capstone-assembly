	...
	const renderContextBadge = (key: string, entry: ContextEntry) => {
		...
		return (
			<div
				...
				onClick={() => {
					// Remove the context entry (works for all sources)
					removeContextEntry(key, entry.id);

					// If it was added via mention, also remove the mention from the editor
					if (entry.source === 'mention' && editor) {
						const { state } = editor;
						const { doc, tr } = state;
						let found = false;
				...
				{/* Show X icon on hover for all entries */}
				{entry.metadata?.icon && (
					<>
