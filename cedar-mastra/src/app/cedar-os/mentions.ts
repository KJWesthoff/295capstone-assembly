...
const useStableMentionProvider = (
  factory: () => Parameters<typeof useStateBasedMentionProvider>[0],
  deps: React.DependencyList,
) => {
  const config = React.useMemo(factory, deps);
  useStateBasedMentionProvider(config);
};
...
export function useFindingsMentions() {
  useStableMentionProvider(
    () => ({
      ...
      searchFields: ['summaryHumanReadable', 'owasp', 'cwe'],
  ...
  useStableMentionProvider(
    () => ({
      ...
      color: (owner: ExecutiveSlaOwner) => {
        if (owner.pastSLA > 2) return '#dc2626';
        if (owner.pastSLA > 0 || owner.critOpen > 0) return '#ea580c';
        return '#16a34a';
      },
    }),
    [],
